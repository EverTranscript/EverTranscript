...
| 12 | A model change re-runs History. Built: `cluster::claims` (`058bcad`), the queue's recording pause and its persistence-boundary stop (Q190/Q192), the additive `rerun` status block plus `diarize/rerunCancel`, the Registry's re-run surface (`4d403d7`), and the bounded seeding writer `diarize::reseed` (`b554fe0`, `04c9de6`, `e4687f6`). Every seam is written; **nothing calls the writer**, `begin` and `begin_if_the_model_changed` stay unreachable, and the tables stay unregistered. Only activation remains — see below. |
