...
| 12 | A model change re-runs History. Built: `cluster::claims` (`058bcad`), the queue's recording pause and its persistence-boundary stop (Q190/Q192), the additive `rerun` status block plus `diarize/rerunCancel`, the Registry's re-run surface (`4d403d7`), and the bounded seeding writer `diarize::reseed` (`b554fe0`, `04c9de6`, `e4687f6`). and the caller `Core::reseed_for_rerun` (Q215), which threads `plan` → `embed_ranges` → `commit` with reading and embedding outside the write transaction and only the replacement inside one. **It runs before the Meeting is re-diarized, and that order is forced**: `plan` reads the attribution the run overwrites, so seeding afterwards would be refused as `Moved` for every Meeting; seeding first is what gives the run Voiceprints to match against. It is gated on `rerun::is_bulk_work`, which is false on every History in the field because the tables are unregistered — so the path is unreachable, pinned by `a_history_in_the_field_is_never_reseeded`. `begin` and `begin_if_the_model_changed` stay unreachable and the tables stay unregistered. What remains is activation: the startup trigger, the model decision and registration. |
