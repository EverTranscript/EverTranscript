...
**12's groundwork was built and unwired**: `cluster::claims` (`058bcad`,
`c72bb74`) and `store::rerun` with its then-unregistered tables (`8dd6781`).
...
rules responsible. **Measured 2026-09-17 (Q234) and it does not hold as written:**
the third channel rule *is* the Voiceprint match, and nothing rebuilds the
Voiceprint it needs, so a model change leaves only the isolated-mic and dominance
rules standing. On the real History that cost the Operator their name on the four
most recent Meetings. Escalated as Q236.
