...
Status: run on real hardware 2026-08-31 — the detector had never named a process and so had never detected anything; fixed and watched working for Edge, two Cores, and the calendar. Open: no meeting app has been seen holding the microphone, because none is installed on that machine
...
- [ ] **No meeting app has been observed holding the microphone on Windows.** The machine the run happened on has Edge and Teams and nothing else — no Zoom, no VooV, no 腾讯会议, no Chrome, Firefox, Brave, Opera or Arc — so `WINDOWS_EXECUTABLES` remains checked-against-a-competitor's-table but unobserved, exactly as it was, and the per-browser matrix from ticket 09 has one row filled. **Teams is the live risk and is named as unknown rather than guessed:** Teams 2.x on that machine is WebView2-hosted, running one `ms-teams.exe` alongside 24 `msedgewebview2.exe` children, so the process holding its capture session may well be `msedgewebview2.exe` and not the row's `ms-teams.exe` — which is the same shape as `com.microsoft.teams2.modulehost`, the defect Teams already produced once on macOS. It was not settled because settling it needs Teams to actually open the microphone on a real signed-in account. **Do not add `msedgewebview2.exe` to the table on the strength of that paragraph** — it would also match every other WebView2 app, and inventing the name is the mistake this milestone is a record of.
- [ ] **The detector reads only the default capture endpoint.** `microphone_holders` asks `GetDefaultAudioEndpoint(eCapture, eMultimedia)`, so an app recording from a second microphone is invisible to it. Observed as true rather than suspected — the probe enumerates every active capture endpoint and marks which one the detector would see. Whether that matters is a product question nobody has asked yet; it is recorded here so it is not discovered as a bug later.
