...
- [ ] Nothing Ambient is amended honestly in the docs, not quietly: an event title is content, read only under a grant (ADR-0036's own consequence). `CONTEXT.md`'s Meeting Detection entry — still "the product's **single** ambient sense", still "reads state, never content" — is corrected as part of this ticket
