...
- **`CONTEXT.md` was stale against ADR-0036 and has been corrected** (done before this milestone started, since the glossary is normative for the spec). Its Meeting Detection entry called itself "the product's **single** ambient sense" while, eleven entries later, Nothing Ambient already enumerated *two* senses including the calendar — the file contradicted itself. Meeting Detection is now the sense that triggers, Calendar Arming is defined as the one that arms and names, and "reads state, never content" stays where it is still true.
