...
- [ ] **Cross-compiles for `x86_64-pc-windows-msvc`, llama.cpp included — and that is worth what it was worth in M3: nothing about runtime.** No Windows machine has run this binary. CI builds the workspace there, so the sidecar is built and linked; what is not proven is that it loads a model and generates. Original criterion: A second binary is a second thing to build, locate and launch on two platforms, and M2 ended by finding a whole platform that had never worked while CI was green
