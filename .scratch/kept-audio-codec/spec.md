...
2. ~~**Disk exhaustion mid-Meeting.**~~ **Done 2026-09-05:** the sink checks
   free space every 30s of audio and stops keeping it below a 1 GiB floor,
   with the reason reaching `audio_notes` — ADR-0019's answer, and stopping
   before the disk is full rather than when it is, because a disk at 100%
   costs the machine and not just the Meeting. Original note: Much smaller now, but not zero — a silent recorder still grows a file nobody is watching. Neither competitor guards this at all (anarlog propagates the write error and fails its recorder actor; Meetily has nothing), and this product already has the parts: `models::free_space_bytes()` and the `checked_add(HEADROOM)` pattern at `provision.rs:76`. The behaviour that matches ADR-0019 is to stop writing audio, keep transcribing, and put the reason in `audio_notes` — which is what the sink already does when its encoder will not start.
