...
function RegistryPanel({
  ...
  const voiceprintLabel = (speaker: Speaker): string => {
    ...
    if (!speaker.hasVoiceprint) {
      // Three sentences, because three situations read differently: one the
      // Operator chose, one a model change caused, and one where there is
      // simply no Voiceprint. The third is deliberately shared — a voice
      // never enrolled and one whose recomputation came back empty are in
      // the same position, and inventing a fourth sentence would draw a
