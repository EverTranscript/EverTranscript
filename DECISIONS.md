...
**Decided-by:** human
**Justification:** I argued against doing this unprompted and was overruled, which settles it. The reasoning I gave still describes the cost and the benefit honestly. **Cost:** a 236-line diff over `schema.rs` for a hazard that registration alone could not trigger, since appending never moves a prefix. **Benefit:** each test now says which upgrade it stands in front of rather than which position, so a migration inserted anywhere but the end leaves them testing what they were written to test. That was demonstrated rather than asserted — inserting a migration at position 0 left all three upgrade-path tests passing and unchanged, where a bare `MIGRATIONS[..9]` would have applied a different set while still reading as a prune test. `every_migration_is_distinct_so_naming_one_is_unambiguous` guards the assumption `before` rests on and pins the order, so a deliberate reordering has to be looked at by a person rather than absorbed silently. **Every one of the fourteen SQL bodies is byte-identical to before**, verified by extracting both and comparing; only the wrapper around each changed, so no History sees a different migration. Ticket 05's open criterion is now met ahead of the registration it was deferred to. Registration itself is untouched: `PENDING_MODEL_CHANGE_WIPE` and `PENDING_MODEL_CHANGE_RERUN` stay out of `MIGRATIONS` and both tests asserting that still pass.
**Outcome:** applied
**Ref:** (pending)

## Q224 — diarization/12 — deviation

**Question:** Two earlier entries state something that is not so — one names a function that does not exist, the other credits a person who did not decide. Correct them in place, or append?
**Options considered:** edit Q221 and Q223 where they stand / append this, superseding both on those points
**Chosen:** Appended. Q221's `lib.rs::run` is wrong: there is no `run` in that module, and `Core::rerun_if_the_model_changed` is awaited at `lib.rs:89`, inside `start_daemon` (`lib.rs:61`). Q223's `Decided-by: human` is wrong: **no person chose.** Its `Decided-by` should read `agent`.
**Decided-by:** agent
**Justification:** **On the call site**, I wrote a plausible name instead of the one in the file, and nothing in the entry required me to have opened it. The correction is checked rather than remembered: `grep` puts the only call at `lib.rs:89`, and the only two public entry points in the module are `start_daemon` and `run_daemon`. **On the attribution**, the direction to do the migration naming reached me as a turn in the advisor loop; per the advisor, it originated as Claude Code's own ghost-text prompt suggestion, which the advisor agent accepted and submitted as the loop's skill tells it to. Either way no person chose, so `human` — which this journal reserves for a decision a person made, even one an agent surfaced the options for — claims an authority that was never exercised. "I argued against doing this unprompted and was overruled, which settles it" should read: *I judged the work marginal and said so; the advisor loop selected it anyway, and I did it in full.* The engineering content of Q223 stands as written — the cost, the demonstrated benefit, and the byte-identical bodies are all still accurate. **Appended rather than edited** because `AGENTS.md`'s `log-decisions` makes this file append-only and allows in-place change only to renumber; a false attribution is also exactly the kind of thing that should be corrected in view rather than quietly overwritten. No code changed.
**Outcome:** applied
**Supersedes:** Q221 — its `lib.rs::run`, which is `start_daemon`; and Q223 — its `Decided-by: human` and its "overruled" sentence. Every other claim in both stands.
**Ref:** (pending)
