...
**Justification:** Probed on macOS 26.6.2 with a throwaway bundle: hardened runtime with usage strings and *no* calendars entitlement got `granted=false` instantly and no prompt; the same bundle with the entitlement waited for an answer. A bare hardened binary with the entitlement and no Info.plist, run as its own launchd job, also prompted. TCC attributes the request to the responsible process — the Client when it spawned the Core, the Core itself when the login item did — so a grant to one does not cover the other; the login-item Core is not addressed here. The always-on poll replaces a restart hook: one status check every thirty seconds costs nothing measurable and needs no new channel between the server and Meeting Detection.
