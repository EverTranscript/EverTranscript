...
> **Amended 2026-08-27 (staging reversed):** Windows is promoted to **simultaneous v1** — the fast-follow staging below is overturned, eyes-open against the scope-trap warning it records. Every milestone closes on both platforms; the platform-trait work distributes into the spine (WASAPI capture + named-pipe transport in M1; Win32 detection, per-process mic attribution — Granola's mined `mic_monitor_v2` as prior art — and the WinRT appointment store in M2; Credential Manager in M4; signed installer + winget in M5). The ship gate is feature parity: **v1 ships only when detection works on Windows** — anarlog's shipped-but-hollow Windows build is the named anti-goal.
